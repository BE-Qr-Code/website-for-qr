CREATE DEFINER=`root`@`%` TRIGGER `test`.`qrcode_BEFORE_INSERT` BEFORE INSERT ON `qrcode` FOR EACH ROW
BEGIN
SET NEW.currDate = SYSDATE();
SET NEW.currTime = SYSDATE();
SET NEW.endtime = ADDTIME(NEW.currTime, "00:01:00");
END
                ...
                if ($result = mysqli_query($connection, "SELECT count(*), DepartAID, SubjectID FROM attendance where DepartAID='$deptid' AND SubjectID='$selected' GROUP BY DepartAID,SubjectID ;")) {
                  if (mysqli_num_rows($result) > 0) {
                    ...
                    if ($result1 = mysqli_query($connection, "SELECT count(MoodleID) from registration where DepartID = (select DepartID from Subjects where SubjectID='$selected');")) {
                      ...
                      $r = $row1[0] - $row[0];
                      $per = (int)(($row[0] / $row1[0]) * 100);
                      $monthval = "SELECT qrdate from qrcode where SubjectQID=$row[2] ORDER BY qrdate DESC limit 1  ;";
...
<div class="containerop">
  <!-- <label for="subjectid" class="labelclass">TOTAL:</label> -->
  <input type="text" class="input4" value="<?php echo ("PERCENTAGE:" . 0 . '%'); ?>" readonly>
</div>
<!-- </div> -->
<?php
                  }
