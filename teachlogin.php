...
function get_dept($moodleId){
    include 'connect.php';
    if ($result = mysqli_query($connection, "SELECT s.DepartSID, t.teacherName FROM Subjects s,teacher t where s.TeacherID=t.teacherID AND t.teacherID='$moodleId';")) {
...
$stmt=$connection->prepare("SELECT teacherID,teacherpasswd FROM teacher WHERE teacherID=? AND teacherpasswd=?");
